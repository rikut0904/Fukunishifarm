...
func (s *Service) CreateUser(ctx context.Context, sessionToken, email, displayName string) (*domainauth.AdminUser, error) {
	...
	if s.mailer == nil || s.loginURL == "" {
		slog.WarnContext(ctx, "create admin user mail is not configured", "email", email, "has_mailer", s.mailer != nil, "login_url", s.loginURL)
		return nil, domainauth.ErrMailNotConfigured
	}

	if _, err := s.GetSession(ctx, sessionToken); err != nil {
		slog.WarnContext(ctx, "create admin user unauthorized", "email", email, "error", err)
		return nil, err
	}

	identity, err := s.creator.CreateInvitedUser(ctx, email, displayName)
	if err != nil {
		slog.WarnContext(ctx, "create invited firebase user failed", "email", email, "error", err)
		return nil, fmt.Errorf("create firebase user: %w", err)
	}

	user, err := s.repository.CreateAdminUser(ctx, identity)
	if err != nil {
		slog.WarnContext(ctx, "create admin user record failed", "email", identity.Email, "firebase_uid", identity.FirebaseUID, "error", err)
	...
	if err != nil {
		slog.ErrorContext(ctx, "generate password setup link failed", "email", identity.Email, "login_url", s.loginURL, "error", err)
		rollbackInvite(ctx, s.creator, s.repository, identity.FirebaseUID)
	...
	if err := s.mailer.SendInvitationEmail(ctx, identity.Email, subject, body); err != nil {
		slog.ErrorContext(ctx, "send invitation email failed", "email", identity.Email, "error", err)
		rollbackInvite(ctx, s.creator, s.repository, identity.FirebaseUID)
...
func (s *Service) ResendInvitation(ctx context.Context, sessionToken string, userID uint) error {
	...
	if s.mailer == nil || s.loginURL == "" {
		slog.WarnContext(ctx, "resend invitation mail is not configured", "user_id", userID, "has_mailer", s.mailer != nil, "login_url", s.loginURL)
		return domainauth.ErrMailNotConfigured
	}

	if _, err := s.GetSession(ctx, sessionToken); err != nil {
		slog.WarnContext(ctx, "resend invitation unauthorized", "user_id", userID, "error", err)
		return err
	}
	...
	if err != nil {
		...
		slog.ErrorContext(ctx, "find admin user by id failed", "user_id", userID, "error", err)
		return fmt.Errorf("find admin user by id: %w", err)
	}
	if !user.LastLoginAt.IsZero() {
		slog.WarnContext(ctx, "resend invitation rejected for active user", "user_id", userID, "email", user.Email)
		return ErrInvalidInput
	}

	passwordSetupLink, err := s.creator.GeneratePasswordSetupLink(ctx, user.Email, buildLoginContinueURL(s.loginURL, user.Email))
	if err != nil {
		slog.ErrorContext(ctx, "generate password setup link failed", "user_id", userID, "email", user.Email, "login_url", s.loginURL, "error", err)
	...
	if err := s.mailer.SendInvitationEmail(ctx, user.Email, subject, body); err != nil {
		slog.ErrorContext(ctx, "send invitation email failed", "user_id", userID, "email", user.Email, "error", err)
