...
アプリケーション用の環境変数は GitHub Secrets ではなく、Vercel / Railway / Cloudflare の各環境に登録します。migration の `DATABASE_URL` のみ GitHub Environment Secrets を使います。Vercel は `vercel pull` で Vercel 側の環境変数を取得します。Railway backend は GitHub Actions から Railway CLI の `railway up --ci` でデプロイします。Railway の service 名が `Fukunishifarm` 以外の場合は、GitHub Actions Variables に `RAILWAY_SERVICE_NAME` を設定してください。preview は Railway environment `preview-v2` にデプロイします。
