...
Railway backend は GitHub Actions から Railway CLI の `railway up --ci` でデプロイします。Railway の service 名が `Fukunishifarm` 以外の場合は、GitHub Actions Variables に `RAILWAY_SERVICE_NAME` を設定してください。preview は Railway environment `preview-v2` にデプロイします。
...
- `preview`
  - Railway Preview
  - Vercel Preview
  - Trigger: Pull Request の `CI` 成功後に `deploy-preview.yml` を自動実行
- `staging`
  - Railway Staging
  - Vercel Production
  - Trigger: `main` の `CI` 成功後に `deploy-staging.yml` を自動実行 (`migration` と deploy を並行実行) / 手動実行
- `production`
  - Railway Production
