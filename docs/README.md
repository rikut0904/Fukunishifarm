...
- `preview`
  - Railway Preview
  - Vercel Preview
  - Trigger: `CI` 成功後に `deploy-preview.yml` を自動実行
- `staging`
  - Railway Staging
  - Vercel Production
  - Trigger: `main` push 後に `CI` → `migrate.yml` → `deploy-staging.yml` の順で自動実行 / 手動実行
- `production`
  - Railway Production
  - Cloudflare Production
  - Trigger: `deploy-production.yml` / 手動実行 (`migration` → Railway → Cloudflare)

## Migration Workflows

- `migrate.yml`
  - `staging` / `production` の 2 環境に対応
  - `main` の `CI` 成功後は `staging` migration を自動実行
