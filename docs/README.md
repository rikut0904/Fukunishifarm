...
- Frontend API
  - `API_INTERNAL_BASE_URL` (preview / staging / production)
- microCMS
  - `MICROCMS_SERVICE_DOMAIN` (preview / staging / production)
  - `MICROCMS_BLOG_ENDPOINT` (任意。未設定時は `blogs`)
  - `MICROCMS_NEWS_ENDPOINT` (任意。未設定時は `news`)
- Cloudflare
  - `CLOUDFLARE_API_TOKEN`
  - `CLOUDFLARE_ACCOUNT_ID`

frontend から backend へ接続するための `API_INTERNAL_BASE_URL`、frontend の microCMS 管理リンク用環境変数、migration の `DATABASE_URL` は GitHub Environment Secrets を使います。CD では `API_INTERNAL_BASE_URL` から `NEXT_PUBLIC_API_BASE_URL` も生成し、Vercel / Cloudflare の build に渡します。microCMS の管理リンク用の値は Vercel / Cloudflare build に渡し、Railway backend の Variables にも deploy 前に同期します。`MICROCMS_API_KEY` は backend/Railway 側の環境変数として管理します。その他のアプリケーション用の環境変数は Vercel / Railway / Cloudflare の各環境に登録します。Vercel は `vercel pull` 後に GitHub Secrets の API URL を build 用 env ファイルへ追加します。Railway backend は GitHub Actions から Railway CLI の `railway up --ci` でデプロイします。Railway の service 名が `Fukunishifarm` 以外の場合は、GitHub Actions Variables に `RAILWAY_SERVICE_NAME` を設定してください。preview は Railway environment `preview-v2` にデプロイします。
...
- `preview`
  - Railway Preview
  - Vercel Preview
  - Trigger: Pull Request 更新時に `deploy-preview.yml` を自動実行
- `staging`
  - Railway Staging
  - Vercel Production
  - Trigger: `main` push 時に `deploy-staging.yml` を自動実行 (`migration` と deploy を並行実行) / 手動実行
- `production`
  - Railway Production
  - Cloudflare Production
  - Trigger: `deploy-production.yml` / 手動実行 (`migration` と deploy を並行実行)
...
- `migrate.yml`
  ...
  - staging / production deploy 時の migration は各 deploy workflow 内で並行実行
